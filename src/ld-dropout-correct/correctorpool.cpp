...
bool CorrectorPool::process()
{
    ...
    metaData[0]->write(outputMetadataFilename);
                                  ...
                                  QVector<qint32>& availableSourcesForFrame, QVector<double>& sourceFrameQuality)
{
    ...
    for (qint32 sourceNo = 0; sourceNo < numberOfSources; sourceNo++) {
        ...
        if (sourceNo == 0) {
            ...
            double firstFrameSnr = metaData[sourceNo]->getField(firstFieldNumber[sourceNo]).vitsMetrics.bPSNR;
            double secondFrameSnr = metaData[sourceNo]->getField(secondFieldNumber[sourceNo]).vitsMetrics.bPSNR;
...
QVector<qint32> CorrectorPool::getAvailableSourcesForFrame(qint32 vbiFrameNumber)
{
    ...
    for (qint32 sourceNo = 0; sourceNo < sourceVideos.size(); sourceNo++) {
        if (vbiFrameNumber >= sourceMinimumVbiFrame[sourceNo] && vbiFrameNumber <= sourceMaximumVbiFrame[sourceNo]) {
            ...
            qint32 firstFieldNumber = metaData[sourceNo]->getFirstFieldNumber(convertVbiFrameNumberToSequential(vbiFrameNumber, sourceNo));
            qint32 secondFieldNumber = metaData[sourceNo]->getSecondFieldNumber(convertVbiFrameNumberToSequential(vbiFrameNumber, sourceNo));

            // Ensure the frame is not a padded field (i.e. missing)
            if (!(metaData[sourceNo]->getField(firstFieldNumber).pad &&
                  metaData[sourceNo]->getField(secondFieldNumber).pad)) {
